...
export const MvyResearchDashboard: React.FC<{ uiLang?: 'en' | 'ur' }> = ({ uiLang = 'en' }) => {
  ...
  return <section className="mx-auto w-full max-w-6xl px-4 py-8 sm:py-12">
    ...
    <div className="mt-7 grid grid-cols-2 sm:grid-cols-4 gap-px overflow-hidden rounded-xl border border-zinc-800 bg-zinc-800">
      {ur ? [['لسانی ذخیرہ',''],['صوتی ڈیٹا',''],['لسانی تحقیق',''],['عوامی وسائل','']] : [['Corpus',''],['Speech data',''],['Language research',''],['Public resources','']].map(([a,b])=><div key={b} className="bg-zinc-950 px-4 py-4"><div className="text-sm text-zinc-200">{a}</div><div className="mt-1 text-[11px] text-zinc-500">{b}</div></div>)}
    </div>
    <div className="mt-8">
      <div className="flex items-baseline justify-between"><h2 className="text-base font-medium text-zinc-200">{ur ? 'تحقیقی وسائل' : '{ur ? 'تحقیقی وسیلہ' : 'Research resource'}s'}</h2><span className="text-[11px] text-zinc-600">{index?.schema_version ? `{ur ? `فہرست ${index.schema_version}` : `catalogue ${index.schema_version}`}` : ''}</span></div>
